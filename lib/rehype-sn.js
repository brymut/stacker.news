...
export default function rehypeSN (options = {}) {
  const { stylers = [], mentionResolver } = options
  ...
  function replaceMention (value, username) {
    // username may contain a path suffix (e.g. name/subpath), but we resolve by the first segment
    const token = (username || '').split('/')[0]
    const resolved = mentionResolver ? mentionResolver(token) : null
    const currentName = resolved || token

    return {
      type: 'element',
      tagName: 'mention',
      // show the original text as-is but link/popover to the current name
      properties: { href: '/' + currentName, name: currentName },
